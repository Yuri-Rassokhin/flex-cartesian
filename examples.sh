...
# Check OPENAI_API_KEY - it's required in several test examples
if [ -z "$OPENAI_API_KEY" ]; then
  echo "❌ Environment variable OPENAI_API_KEY is not set, please set it to your OpenAI API token"
  exit 1
fi
...
ex="15_dimensions 16_ping_temporal 17_rag_optimization"
