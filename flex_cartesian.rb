...
puts "Iterator s.cartesian { |v| ... } traversing three-dimensional Cartesian:"
s = FlexCartesian.new( { dim1: [1,2,3], dim2: ["qwe","rty" ], dim3: [true, false] })
s.cartesian { |v| puts v.dim2 + v.dim1.to_s if v.dim3  }

puts "Iterator s.cartesian { |v| ... } traversing two-dimensional Cartesian:"
s = FlexCartesian.new( { dim1: [1,2,3], dim2: [true, false] })
s.cartesian { |v| puts v.dim1 if v.dim2  }

puts "Standard iterator each is applicable as well:"
e = s.cartesian
e.each { |v| puts v.dim1 }

puts "Calculate Cartesian size: #{s.size}"

puts "Lazily take two Cartesian combinations without materializing entire Cartesian:"
s.cartesian(lazy: true).take(2).each do |v|
  puts "Quick Cartesian combinations: #{v.inspect}"
end
...
# convert Cartesian to array of combinations, with optinal limit for the number of combinations
puts "Convert first 3 combinations to array: #{s.to_a(limit: 3).inspect}"

puts "Iterate through Cartesian with progress bar:"
s.progress_each do |v|
  sleep(0.1)  # do something
end

puts "Output entire Cartesian value:"
s.output(align: true)

puts "Output Cartesian combinations in a loop:"
s.cartesian { |v| v.output(align: true, colorize: true) }

puts "Creating example JSON file to read Cartesian from: 'config.json'"
config = {
  dim1: [1, 2],
  dim2: ['x', 'y'],
  dim3: [true, false]
...
puts "Creating Cartesian object from the file 'config.json':"
s = FlexCartesian.from_json("config.json")
s.output(align: true)
```
