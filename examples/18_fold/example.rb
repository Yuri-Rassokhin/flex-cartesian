...
s.func(:run, progress: true, title: "Pinging #{s.size} times")

s.output

# Fold the 'iteration' dimension using average values - this remove random noise of the benchmark
report = s.fold(:iteration, func: [ :time, :min ] ) { |v| (v.map(&:to_f).sum / v.size).round(2) }
