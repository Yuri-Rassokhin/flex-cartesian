...
s.func(:add, :time) { |v| print "#{counter} "; counter+=1; v.raw_ping[/min\/avg\/max\/(?:mdev|stddev) = [^\/]+\/([^\/]+)/, 1]&.to_f.round(2) } # fetch ping time from result
...
puts "Adding temporal dimension `iteration`"
